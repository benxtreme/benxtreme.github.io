...
function select(selector) {
  if (selector instanceof Element) return selector;else if (typeof selector === 'string') return document.querySelector(selector);
  return null;
}
...
function scrollama() {
  ...
  function handleEnable(enable) {
    if (enable && !isEnabled) {
      ...
      if (io.stepAbove) io.stepAbove.forEach(function (d) {
        return d.disconnect();
      });
      if (io.stepBelow) io.stepBelow.forEach(function (d) {
        return d.disconnect();
      });
      if (io.stepProgress) io.stepProgress.forEach(function (d) {
        return d.disconnect();
      });
      if (io.viewportAbove) io.viewportAbove.forEach(function (d) {
        return d.disconnect();
      });
      if (io.viewportBelow) io.viewportBelow.forEach(function (d) {
        return d.disconnect();
      });
      isEnabled = false;
    }
  ...
  function notifyStepEnter(element, direction) {
    ...
    if (progressMode) {
      if (direction === 'down') notifyStepProgress(element, 0);else notifyStepProgress(element, 1);
    }
  }
  ...
  function notifyStepExit(element, direction) {
    ...
    if (progressMode) {
      if (direction === 'down') notifyStepProgress(element, 1);else notifyStepProgress(element, 0);
    }
  ...
  function notifyContainerEnter() {
    ...
    if (callback.containerEnter && typeof callback.containerEnter === 'function') callback.containerEnter(resp);
  }
  ...
  function intersectStepAbove(entries) {
    ...
    entries.forEach(function (entry) {
      ...
      if (bottomAdjusted >= -ZERO_MOE) {
        if (isIntersecting && direction === 'down' && ss.state !== 'enter') notifyStepEnter(target, direction);else if (!isIntersecting && direction === 'up' && ss.state === 'enter') notifyStepExit(target, direction);else if (!isIntersecting && bottomAdjusted >= height && direction === 'down' && ss.state === 'enter') {
  ...
  */
  ...
  function intersectTop(entries) {
    ...
    if (bottom > -ZERO_MOE) {
      if (isIntersecting) notifyContainerEnter(direction);else if (containerState.state === 'enter') notifyContainerExit(direction);
    }
  }
...
var FancyTextInput = function () {
  ...
  createClass(FancyTextInput, [{
    ...
    value: function setHeight(e) {
      ...
      e.srcElement.style.height = ""; // Reset height (for when line count decreases)
...
stickybits('.npc__contents__sidebar__inner', { stickyBitStickyOffset: 50 });
stickybits('.npc__contents__sidebar__footer', { verticalPosition: 'bottom', stickyBitStickyOffset: 20 });
...
function initStepWatcher() {
  ...
  scroller.setup({
    ...
    if (window.smoothAnchorJumping) return;
...
(function ($, Drupal, drupalSettings) {
  alertInit();
  guideInit(); //shoudl be before ambient video
  siteInit();
  modalInit();
  accordionInit($);
  // accordionShare();
